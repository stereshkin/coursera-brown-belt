...
struct Address {
    ...
    bool operator==(const Address& other) const {
        return tie(this->city, this->street, this->building) == tie(other.city, other.street, other.building);
    }
};
...
struct Person {
    ...
    bool operator==(const Person& other) const {
        return tie(this->name, this->height) == tie(other.name, other.height)
        and abs(this->weight - other.weight) < 1e-3 and this->address == other.address;
    }
};

struct AddressHasher {
    size_t operator() (const Address& address) {
...
struct PersonHasher {
    size_t operator() (const Person& p) const {
        ...
        const int x = 71;
        return string_hasher(p.name)*x*x*x + int_hasher(p.height)*x*x + double_hasher(p.weight)*x + address_hasher(p.address);
    }
