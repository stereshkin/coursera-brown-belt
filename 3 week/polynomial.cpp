...
public:
    ...
    public:
        ...
        Polynomial<T> operator=(T value) {
            if (value > 0) {
                this->poly_.coeffs_.resize(degree_+1);
                this->poly_.coeffs_[degree_] = value;
                return this->poly_;
            }
            return this->poly_;
        }
        bool operator==(const T& value) {
            return this->poly.coeffs_[degree_] == value;
        }

        ostream& operator<<(ostream& os) {
            os << "Degree: " << degree_ << endl;
            for (const auto& c : poly_.coeffs_) {
                os << c << " ";
            }
            os << endl;
            return os;
...
// Вызываем константную версию Polynomial::operator[]
    ...
    IndexProxy operator[](const int value){

        return {*this, static_cast<size_t>(value)};
    }
